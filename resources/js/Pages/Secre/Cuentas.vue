...
import FormularioCuentas from '@/Components/secre/FormularioCuentas.vue';
...
const restaurarUsuario = (usuario) => {
    ...
    swal.fire({
        ...
        if (result.isConfirmed) {
            try {
                await form.put(route('secre.restUsuario', idUsuario));
            } catch (error) {
                console.log('El error se origina aquí');
                console.log(error);
            }
...
<template>
    <SecreLayout title="Cuentas" :usuario="props.usuario">
