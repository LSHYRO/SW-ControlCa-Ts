...
import FormularioMateria from '@/Components/secre/FormularioMateria.vue';
...
const eliminarMateria = (idMateria, materia) => {
    ...
    swal.fire({
        ...
        if (result.isConfirmed) {
            form.delete(route('secre.eliminarMaterias', idMateria));
        }
    ...
    swal.fire({
        ...
        if (result.isConfirmed) {
            try {
                ...
                await form.delete(route('secre.elimMaterias', $materiasIds));
...
<template>
    <SecreLayout title="materias" :usuario="props.usuario">
